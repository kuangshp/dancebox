...
type WeiXinUserEntity struct {
	...
	Gender           int64     `gorm:"gender" json:"gender"`                       // 性别
	...
	Status           int64     `gorm:"status" json:"status"`                       // 状态,0不正常,1正常
	...
	Unionid          string    `gorm:"unionid" json:"unionid"`                     // 微信小程序授权的unionid
	SessionKey       string    `gorm:"session_key" json:"sessionKey"`              // 微信小程序授权的sessionKey
	IsRead           int64     `gorm:"is_read" json:"isRead"`                      // 是否阅读购票协议,0表示未阅读,1表示已经阅读
	...
	AvatarBanner     LocalList `gorm:"avatar_banner" json:"avatarBanner"`          // 自动轮播头像
	RoleTags         LocalList `gorm:"role_tags" json:"roleTags"`                  // 角色标签
	StyleDomain      LocalList `gorm:"style_domain" json:"styleDomain"`            // 风格领域
...
func (t *WeiXinUserEntity) TableName() string {
	return "wei_xin_user"
}
