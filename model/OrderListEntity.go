...
type OrderListEntity struct {
	...
	Status                int64     `gorm:"status" json:"status"`                                  // 订单票务状态,0表示未支付,1表示已支付,2表示支付失败,3表示取消订单,4表示退款(已经退款),5表示已经核销, 6表示在支付等待支付回调,7表示申请退款(还没退钱),8:拒绝申请退款, 9:退款失败
	Remark                string    `gorm:"remark" json:"remark"`                                  // 备注信息
	OrderId               int64     `gorm:"order_id" json:"orderId"`                               // 关联的订单id
	VerificationId        int64     `gorm:"verification_id" json:"verificationId"`                 // 核销员ID
	VerificationAt        LocalTime `gorm:"verification_at" json:"verificationAt"`                 // 核销时间
	RealUserAddress       string    `gorm:"real_user_address" json:"realUserAddress"`              // 实名的性别
	ActivityEndTime       LocalTime `gorm:"activity_end_time" json:"activityEndTime"`              // 活动结束时间
	...
	IsVerification        int64     `gorm:"is_verification" json:"isVerification"`                 // 是否核销,0表示未核销,1表示已核销,2表示票品过期
	RealSerTeamName       string    `gorm:"real_ser_team_name" json:"realSerTeamName"`             // 战队名称
	ActivityStartTime     LocalTime `gorm:"activity_start_time" json:"activityStartTime"`          // 活动开始时间
	ApplyStartDateTime    LocalTime `gorm:"apply_start_date_time" json:"applyStartDateTime"`       // 申请退票时间
	ConfirmRefundDateTime LocalTime `gorm:"confirm_refund_date_time" json:"confirmRefundDateTime"` // 确认退款时间
	CloseRefundDateTime   LocalTime `gorm:"close_refund_date_time" json:"closeRefundDateTime"`     // 退款关闭时间
	FailRefundDateTime    LocalTime `gorm:"fail_refund_date_time" json:"failRefundDateTime"`       // 退款失败时间
